...
# `make test` directly, or build a target directly, eg. `nix-build -A where
#
SHELL		= bash
DNANAME		= where
DNA		= $(DNANAME).dna
HAPP		= $(DNANAME).happ
WASM		= target/wasm32-unknown-unknown/release/where.wasm
...
update-release-%:
	cd zomes/where/ && sed -i -e 's/^version = .*/version = "$*"/' Cargo.toml
...
update-hc-sha:
	@if [ $(HC_REV) ]; then\
		echo "⚙️  Updating where using holochain rev: $(HC_REV)";\
...
github-release-%:
	...
	cp where.happ where.$(shell echo $* | tr .- _).happ
	cp where.dna where.$(shell echo $* | tr .- _).dna
